...
def build_calc_sheet(wb, k, maxrows):
    ...
    checks = [
        ...
        (11, "選択スタッフの一覧件数（欠勤＋当日時間変更、このシート）", f'=IF({dash}!$AC$3="",0,COUNTIFS({arng},{dash}!$AC$3,$I$2:$I${last},1)+COUNTIFS({arng},{dash}!$AC$3,$BB$2:$BB${last},1))'),
    ...
    for r in range(2, last + 1):
        f = {
            ...
            "W": (f'=IF($A{r}="","",IF(AND(OR($I{r}=1,$BB{r}=1),$A{r}={dash}!$AC$3),'
                  f'COUNTIFS($A$2:$A${last},$A{r},$I$2:$I${last},1,$B$2:$B${last},"<"&$B{r})'
                  f'+COUNTIFS($A$2:$A${last},$A{r},$BB$2:$BB${last},1,$B$2:$B${last},"<"&$B{r})+1+$AA$12,""))'),
        ...
        for c in ("AV", "AW", "AX", "BA", "BC"):      # 日付を含む式は日付書式を引き継ぐので、キー・フラグは数値書式に固定
...
def build_dashboard(wb, maxrows, select=None, pdf=False):
    ...
    #      BL 却下の行数 / BM 同職種の募集件数（誰かがその日その職種に入っているか）
    #      区分 4=出勤 3=当日欠勤 2=事前に休み 1=却下 0=シフト希望未提出 -1=募集なし（対象外）
    hol = q(S_HOL)
    ...
    for i in range(BUSY_DAYS):
        ...
        hws[f"AT{h}"] = (f'=IF(OR($AP{h}=0,{sel}=""),"",IF(N($BM{h})=0,-1,'
        ...
        hws[f"AZ{h}"] = f'=IF(OR($AV{h}=0,{sel}=""),"",IF(AND(N($AQ{h})=0,N($BM{h})>0),1,0))'
        ...
        # 同職種の募集が誰かに入っているか（0なら、その日は本人が出しようのない日として対象外にする）
        hws[f"BM{h}"] = (f'=IF(OR($AV{h}=0,{sel}="",{P}${HC}$5=""),"",'
                         + "+".join(f'COUNTIFS({calc_rng(k, "B", maxrows)},$AN{h},{calc_rng(k, "R", maxrows)},{P}${HC}$5)'
                                    for k in range(1, NSHEETS + 1)) + ")")
    ...
    for rr, label, formula in [(96, "繁忙日（対象期間内・募集のあった日）",
                                f'=COUNTIF($AP${BUSY_R0}:$AP${BUSY_LAST},1)-COUNTIF({BT},-1)'),
                               (97, "　うち出勤", f"=COUNTIF({BT},4)"),
                               (98, "　うち当日欠勤", f"=COUNTIF({BT},3)"),
                               (99, "　うち事前に休み", f"=COUNTIF({BT},2)"),
                               (61, "　うち却下（店舗側が断った）", f"=COUNTIF({BT},1)"),
                               (62, "　うち募集なし（対象外）", f"=COUNTIF({BT},-1)"),
    ...
    def absence_block(top):
        ...
        hr = top + 2
        ws.row_dimensions[hr].height = 18
        list_cols = [("日付", 2, 4), ("曜", 5, 5), ("区分", 6, 9), ("変更した日時", 10, 12), ("元のシフト時間", 13, 16), ("変更後", 17, 20), ("募集の職種", 21, 24)]
    ...
    def busy_block(top):
        ...
        ws[f"B{sr}"] = (f'=IF({sel}="","",IF(N({P}${HC}$96)=0,'
                        ...
                        f'&IF(N({P}${HC}$62)>0,"　（ほかに同じ職種の募集が無かった日 "&N({P}${HC}$62)&"日は対象外）","")))')
        ...
        ws[f"B{r}"] = (f'=IF({sel}="","",IF(N({P}${HC}$58)>{BUSY_ROWS},'
                       ...
                       f'（同じ職種の募集が誰にも入っていない日は、出しようがないので対象外にしています）。'
